...
```{r nonspawnertable2}
seltable <- makeapptable(dat, nullmod, basemod, nullformula, baseformula, resp, vars$covnamelist, vars$covlags, include.num = FALSE)
```
...
chlbody <- makeapptable(dat, nullmod, basemod, nullformula, baseformula, resp, vars$covnamelist, vars$covlags, m3form=m3form, include.num = FALSE)
...
seltable = rbind(seltable, spacetmp)
tabbody <- makeapptable(dat, nullmod, basemod, nullformula, baseformula, resp, covnamelist, covlags, include.num = FALSE)
...
thecap=paste("Covariate tests for the Oct-Mar catch ($W_t$). M is the base model with prior season Oct-Mar catch ($W_{t-1}$) and Jul-Sep catch two seasons prior ($S_{t-2}$) as the covariates. To the base model, the  environmental covariates are added. See Table A1 for further details.")
